        ...
        value: function buildRegistry() {
            ...
            this.registryFunkArray.forEach(function (x) {
                ...
                if (reg.dependencyDeclarations.length > 0) {
                    registry.dependencyDeclarations = registry.dependencyDeclarations.length <= 0 ? reg.dependencyDeclarations : registry.dependencyDeclarations.concat(reg.dependencyDeclarations);
                }
                if (reg.renamedDeclarations.length > 0) {
                    registry.renamedDeclarations = registry.renamedDeclarations.length <= 0 ? reg.renamedDeclarations : registry.renamedDeclarations.concat(reg.renamedDeclarations);
                }
            });
            return registry;
        }
